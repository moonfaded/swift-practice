...
func positive(_ arr: [Int]) -> Int {
    arr.filter{ $0 > 0 }.reduce(0, +)
}
...
func repl(_ s: String) -> String {
    var result = s
    let dictForReplacing = ["1":"один", "2":"два", "3":"три", "4":"четыре", "5":"пять",
                            "6":"шесть", "7":"семь", "8":"восемь", "9":"девять", "0":"ноль"]
    dictForReplacing.keys.forEach {
        result = result.replacingOccurrences(of: $0, with: dictForReplacing[$0]!)
    }
    return result
}
print(repl("Мне 12 лет"))
print(repl("Мне 2 года"))
print(repl("Это стоит 10 рублей"))
